...
func GetRegionData(r *gin.Context) {
	log := new(utils.LoggerId)
	log.SetSid(r.Request)
	log.Log("GetRegionData (+)")
	defer log.Log("GetRegionData (-)")
	...
	lErr := database.DB.Table("order_details od").Select("o.region,sum(od.quantity_sold) as quantity").Joins("join products p on p.product_id = od.product_id").Joins("join orders o on o.order_id =od.order_id").
		Where("o.date_of_sale between ? and ?", StartDate, EndDate).Group("o.region").Order("od.quantity_sold desc").Find(&lResponse)
	if lErr.Error != nil {
	...
	response := map[string]any{
		"status":  "S",
		"message": "Top Product For Region",
		"data": map[string]any{
